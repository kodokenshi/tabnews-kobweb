...
class TestContext(
  ...
  fun test(
    name: String,
    testTimeoutInMillis: Long = this.testTimeoutInMillis,
  ) = TestScope(testTimeoutInMillis, name)
  ...
  inner class TestScope(
    ...
    inner class Assertion<T>(
      private val actual: T,
    ) {
      suspend fun toBe(expected: T) = toBeImpl(expected)
      ...
      suspend fun toNotBe(expected: T) = toNotBeImpl(expected)

      suspend fun toBeMissing() = toBeImpl(null)

      suspend fun toBePresent() = if (toNotBeImpl(null)) actual!! else throw AssertionException("assertion")

      private suspend fun toBeImpl(
        expected: Any?,
        message: Any? = expected,
      ) = measureTime(testTimeoutInMillis) {
        if (actual != expected) {
          fail(message)
      ...
      private suspend fun toNotBeImpl(
        expected: Any?,
        message: Any? = expected,
      ) = measureTime(testTimeoutInMillis) {
        if (actual == expected) {
      ...
      private fun fail(expected: Any?) {
        ...
        println(
          """
          $FAIL[FAIL]: $TEXT$name${RESET}${if (measuredTime.inWholeMilliseconds >= REPORT_TIME) {
            " $FAIL(${formatDuration(
              measuredTime,
            )})$RESET"
          } else {
